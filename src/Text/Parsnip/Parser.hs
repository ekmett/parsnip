...
atEnd :: Parser s e Bool
atEnd = Parser \p s -> case readCharOffAddr# p 0# s of
  (# t, c #) -> OK (isTrue# do chr# 0# `eqChar#` c) p t

endOfInput :: Parser s e ()
endOfInput = Parser \p s -> case readCharOffAddr# p 0# s of
  (# t, c #) -> (# if isTrue# do chr# 0# `eqChar#` c then Good () else Bad, p, t #)

take :: forall s e. KnownBase s => Int -> Parser s e ByteString
...
skip :: forall s e. KnownBase s => Int -> Parser s e ()
skip = \(I# i) -> Parser \p s ->
    if isTrue# (minusAddr# r p <# i)
    then Fail p s
...
skipTillSubstring :: ByteString -> Parser s e ()
...
input :: KnownBase s => Parser s e ByteString
input = absolute \b _ -> SimpleOK b 0

-- | @rest = mark >>= \p -> snip p maxBound@
rest :: KnownBase s => Parser s e ByteString
...
pos :: forall s e. KnownBase s => Parser s e Int
pos = Parser \ p s -> OK (I# (minusAddr# p (start @s))) p s
{-# inline pos #-}

loc :: KnownBase s => Parser s e Location
